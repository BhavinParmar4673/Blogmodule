...
$('body').on('click', '#createcategory', function () {
    ...
    $('#modal-category').on('hide.bs.modal', function (e) {
        $('.mydiv img').attr('src','https://www.riobeauty.co.uk/images/product_image_not_found.gif');
    });
});
...
$('body').on('click', '.edit', function () {
    ...
    $.get(url, function (data) {
        ...
        $('input[type="file"]').closest('div').after('<img src="'+data[1].image+'" width="120px"/>');
    });
    $('#modal-category-edit').on('hide.bs.modal', function (e) {
        $('input[type="file"]').closest('div').next().remove()   
    });
});
