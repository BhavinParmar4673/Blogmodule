@php
$image = $priview != '' ? $priview : 'https://via.placeholder.com/120x80.png';
@endphp
<div class="form-group" x-data="uploadImage()" x-init="init('{{ $image }}')">
    <label for="{{ $id ?? $name }}" class="form-label fs-6 fw-bolder mb-3 text-left">{{ $label }}
        @isset($required)
        ...
        <div class="position-relative">
            ...
            <img id="image" class="rounded w-100"
                style="height: {{ $height }}; background-color: #e2e8f0; object-fit: contain;" :src="preview">

            <template x-if="remove == true">
                <span style="top: -5px;right: -5px;cursor: pointer; z-index: 1;"
                    @click="clearPreview('{{ $id ?? $name }}')" class="position-absolute d-flex">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
                        id="Capa_1" x="0px" y="0px" viewBox="0 0 455.111 455.111"
                        style="enable-background:new 0 0 455.111 455.111;" xml:space="preserve" width="15px" height="15px"
                        class="">
                        <g>
                            <circle style="fill:#1273EB" cx="227.556" cy="227.556" r="227.556" data-original="#E24C4B"
                                class="" data-old_color="#E24C4B" />
        ...
        <input type="file" name="{{ $name }}" multiple x-ref="{{ $id ?? $name }}" name="{{ $name }}"
            id="{{ $id ?? $name }}" {{ $required ?? '' }} class="d-none {{ $class ?? '' }}"
